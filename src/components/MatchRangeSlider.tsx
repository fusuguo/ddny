/**
 * 颜色匹配范围固定档位（20 档）：严格（3）→ 宽松（80）。
 * 滑动条与加减按钮均只允许在固定档位间移动，不支持连续数值。
 */
export const THRESHOLD_LEVELS = [3, 7, 11, 15, 20, 24, 28, 32, 36, 40, 44, 49, 53, 57, 61, 65, 69, 74, 78, 80];

/** 默认匹配阈值（保持原相对位置约 1/3 处） */
...
/**
 * 颜色匹配范围滑动条：严格（严格匹配）→ 宽松（宽泛匹配）
 * 20 档固定调节，不直接展示具体数值，避免用户困惑。
  ...
  return (
    <div className="match-range">
      ...
      <div className="range-row">
        <span className="range-end">严格</span>
        <button
          type="button"
          className="step-btn"
          onClick={() => step(-1)}
          disabled={disabled || levelIndex <= 0}
          aria-label="缩小匹配范围（更严格）"
        >
          −
        </button>
        <input
      ...
      <p className="range-hint">严格：匹配更严格，避免识别多余格子，但可能漏掉正确格子<br />宽松：匹配更宽松，避免识别不全，但可能匹配到相近颜色格子</p>
    </div>
  );
}
